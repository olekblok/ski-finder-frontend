...
export default function SkiResortFilter() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Append each selected country as a separate entry in formData
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      <MountainBackground />
      <div className="absolute top-4 left-4 flex items-center space-x-2 text-gray-800">
        <Snowflake className="w-6 h-6" />
        <span className="text-xl font-bold">SkiFinder</span>
      </div>
      <div className="bg-white bg-opacity-90 rounded-lg shadow-lg p-6 w-full max-w-md relative z-10 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full bg-gray-800 hover:bg-gray-700 text-white mt-4">Search</Button>
        </form>
      </div>
    </div>
  )
